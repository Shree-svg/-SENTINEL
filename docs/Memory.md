...
| Prototype scoping | MQ2 + DHT11 only, no MQ4 | No MQ4 sensor available; no safe access to high-pressure methane in an indoor residential setting. DHT11 substitutes as a second sensing dimension for interference discrimination. | Multi-gas fusion deferred. | Reversible: add MQ4, extend feature extraction with multi-sensor ratios per REQ-ML-1. |
